...
::: reaxkit.analysis.force_field.optimization
    options:
...
### Method: `recommended_presentations(_result: ForceFieldOptimizationResult, payload: dict[str, Any])`
...
|---|---|---|
| `_result` | `ForceFieldOptimizationResult` | Typed analyzer result instance (unused by current logic). |
...
### Method: `run(data: ForceFieldOptimizationProgressData, request: ForceFieldOptimizationRequest, reporter=None)`
...
| `request` | `ForceFieldOptimizationRequest` | Request containing optional epoch filters. |
...
| Type | Description |
|---|---|
| `ForceFieldOptimizationResult` | Analyzer result with the normalized progress table. |
...
## Result: `ForceFieldOptimizationResult`
