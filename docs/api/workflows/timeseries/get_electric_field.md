...
# Get Electric Field Workflow

::: reaxkit.workflows.timeseries.get_electric_field
    options:
...
For --xaxis frame, cadence is read from control/iout2, then from xmolout headers, or finally from an explicit --frame-count.

### Examples
-----

```text
  reaxkit get_electric_field --components field_z --fort78 fort.78 --xaxis frame --plot single
```
...
| `--components` | Yes |  |  |  |
| `--field-kind` | No | auto |  | applied, energy, auto |
...
| `--fort78` | No | fort.78 | Electric-field input path. |  |
...
| `--copy-to-dot` | No | False | Also copy explicitly saved plot or CSV artifacts to the current directory. |  |
