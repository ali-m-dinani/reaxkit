...
# Get Partial Energy Workflow

::: reaxkit.workflows.timeseries.get_partial_energy
    options:
...
Use this command to inspect how individual energy contributions change during a simulation.
...
```text
  1. Save every partial-energy component as its own plot:
     reaxkit get-partial-energy --fort73 .\energylog --save partial_energies --plot separate
     Reads all components from energylog and saves one figure per component in the partial_energies directory.
```
...
| `--components` | No |  | Partial-energy components to include. Example: --components Ebond Eatom, which includes only the bond and atomic energy series; omit this option to include all available components. |  |
...
| `--fort73` | No | fort.73 | Partial-energy input path. |  |
