...
# Get Trajectory Workflow

::: reaxkit.workflows.timeseries.get_trajectory
    options:
...
| `--atom-ids` | No |  | 1-based atom IDs; defaults to all atoms. |  |
| `--atom-types` | No |  | Atom types used when atom IDs are omitted. |  |
| `--dims` | No | x, y, z |  | x, y, z, xy, xz, yz, xyz |
