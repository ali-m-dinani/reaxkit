...
- **cache/frames**: source-oriented SQLite stores hold xmolout and fort.7 byte offsets plus parsed frames. Explicit finite selections share overlapping source-frame indices across commands and processes. `--no-input-cache` bypasses this layer, and `--frame-cache-max-gb` controls its least-recently-used generation limit.
...
- **cache/index**: `handlers.json`, `frames.json`, and `analysis.json` index their corresponding cache files. Frame identities include the source generation and parser view, while analysis identities continue to include task and request settings.
