...
def _run_payloads(payloads: Iterable[_FramePayload], request, context, reporter=None):
    ...
    if int(request.workers) == 1:
        for payload in iterator:
            consume(payload, _frame_rows(payload, request, context))
    else:
        with ThreadPoolExecutor(max_workers=int(request.workers), thread_name_prefix="reaxkit-basal") as pool:
            while True:
                chunk = []
                ...
                if not chunk:
                    break
                results = pool.map(lambda item: _frame_rows(item, request, context), chunk)
                for payload, result in zip(chunk, results, strict=True):
                    consume(payload, result)
...
class BasalPlaneProjectedPolarityTask(AnalysisTask):
    ...
    def run_stream(self, frames, request, reporter=None):
        ...
        return _run_payloads(selected_payloads(), request, context, reporter=reporter)
