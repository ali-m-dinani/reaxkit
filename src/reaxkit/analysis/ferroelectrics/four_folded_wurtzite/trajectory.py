"""Extended XYZ export with site polarity as atom properties."""
...
class PolarityExtendedXYZRequest(WurtzitePolarityRequest):
    """Configure an Extended XYZ trajectory containing local-polarity columns."""
...
class PolarityExtendedXYZResult(BaseResult):
    """Metadata for a generated polarity Extended XYZ trajectory."""
...
def _electric_field_metadata(
    ...
    value = float(np.asarray(values, dtype=float)[0]) * float(const("electric_field_VA_to_MVcm"))
    return {
...
def polarity_extended_xyz_frame(
    ...
    """Build one species-preserving Extended XYZ frame with polarity properties."""
    ...
    eta = np.full(len(atom_ids), np.nan, dtype=float)
    delta = np.full(len(atom_ids), np.nan, dtype=float)
    is_site = np.zeros(len(atom_ids), dtype=int)
    complete = np.zeros(len(atom_ids), dtype=int)
    ...
    for _, source in polarity_table.iterrows():
        atom_id = int(source["site_atom_id"])
        index = id_to_index.get(atom_id)
        if index is None:
            continue
        ...
        complete[index] = int(bool(source["has_four_neighbors"]))
    ...
    frame_number = (
        ...
        else int(output_frame_index)
    )
    return ExtendedXYZFrame(
        species=labels[valid].tolist(),
        ...
        properties={
            ...
            "eta_c": eta[valid],
            "delta_eff": delta[valid],
            "is_polarity_site": is_site[valid],
            "has_four_neighbors": complete[valid],
...
@register_task("write-trajectory-with-polarity", label="Write Trajectory with Polarity")
class PolarityExtendedXYZTask(AnalysisTask):
    ...
    def recommended_presentations(
        ...
        return [PresentationSpec(renderer="table", label="Polarity trajectory", view_type="table")]
    ...
    def run(self, data, request: PolarityExtendedXYZRequest, reporter=None):
        ...
        with ExtendedXYZWriter(request._output_path, precision=request.precision) as writer:
            for progress_index, frame in enumerate(selected, start=1):
                ...
                frames.append(int(extended.frame))
