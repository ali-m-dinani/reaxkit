...
def interpret_ffield_terms(
    ...
    *,
    ...
    """
    Return section DataFrames with numeric atom indices converted to symbols.

    Adds:
      - i_symbol / j_symbol / k_symbol / l_symbol (as relevant)
      - term column (e.g. 'C-H', 'C-C-C', 'C-C-C-C', 'O-H-O')

    Parameters
    ----------
    handler
        FFieldHandler instance (parsed or not; this will parse if needed).
    sections
        Which sections to convert. Default: all relevant ones:
        bond, off_diagonal, angle, torsion, hbond
    sep
        Separator used in 'term' (default '-').
    """
    if not isinstance(handler, FFieldHandler):
...
def interpret_one_section(
    ...
    *,
    ...
    """
    Convenience wrapper to interpret just one section (bond/off_diagonal/angle/torsion/hbond).
    """
