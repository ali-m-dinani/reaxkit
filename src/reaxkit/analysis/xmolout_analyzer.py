...
def get_atom_table(xh: XmoloutHandler, frame: int = 0) -> pd.DataFrame:
    """Return a per-atom DataFrame for the selected frame from an XmoloutHandler.

    Provides a convenient, user-facing wrapper over `_frame_table`, ensuring
    a clean copy of the atomic data. The resulting DataFrame includes:
        - atom_type (str)
        - x, y, z coordinates (float)

    Useful for quick inspection, geometric filtering, or passing atomic data
    to downstream analysis or visualization functions.
    """
    return _frame_table(xh, int(frame)).copy()
...
def get_xmolout(
    ...
    *,
    ...
    """Extract structural or box information from an XmoloutHandler as a tidy DataFrame.

    Provides two modes:
      - **"box"** → cell dimensions and thermodynamic quantities per frame.
      - **"traj"** → atomic coordinates and types across selected frames.

    Supports flexible frame, atom, and dimension selection and can return results
    in either long (tidy) or wide format, suitable for analysis or export.

    Behavior
    --------
    - Frame selection:
        * Controlled via `frames` (single, slice, or iterable) and subsampled by `every`.
    - For `"box"`:
        * Returns columns such as `a`, `b`, `c`, `alpha`, `beta`, `gamma`, `E_pot`,
          and `num_of_atoms`, if available.
    - For `"traj"`:
        * Returns atom-resolved data including atom IDs, types, and coordinates.
        * Atom selection by index (`atoms`) or type (`atom_types`).
        * Coordinate dimensions filtered via `dims` (`x`, `y`, `z`).
        * Output format:
            - `"long"` → one row per atom per frame.
            - `"wide"` → one column per atom/dimension (e.g., `x[1]`, `y[1]`, ...).
    - If `df_sim` is empty (e.g., no parsed frames), returns an empty structured DataFrame.

    Notes
    -----
    - `what` must be `"box"` or `"traj"`.
    - `dims` must include at least one of `x`, `y`, or `z`.
    - The function preserves iteration indices if available in the handler DataFrame.
    - Internally uses `_df_select()` and `xh.frame()` for efficient parsing.
    - Useful for exporting geometry, trajectory slices, or box data for downstream analysis.
    """
    ...
    if what.lower() == "box":
        if df_sim.empty:
            return pd.DataFrame(columns=["frame_index", "iter", "a", "b", "c", "alpha", "beta", "gamma", "E_pot", "num_of_atoms"])
        sub = df_sim.iloc[fidx].copy().reset_index(drop=True)
        sub.insert(0, "frame_index", fidx)
        cols = ["frame_index", "iter", "a", "b", "c", "alpha", "beta", "gamma", "E_pot", "num_of_atoms"]
        return sub[[c for c in cols if c in sub.columns]]

    elif what.lower() == "traj":
        rows = []
        for i in fidx:
            fr = xh.frame(i)
            coords = fr["coords"]
            # Accept both atom index slice and atom_types filtering:
            if atoms is not None:
                if isinstance(atoms, slice):
        ...
        raise ValueError("what must be 'box' or 'traj'")
...
def cut_non_coord_data(
    xh: XmoloutHandler,
    frame: int = 0,
    keep_cols: Sequence[str] = ("atom_type", "x", "y", "z"),
    atoms: AtomSel = None,
    atom_types: Optional[Sequence[str]] = None,
) -> pd.DataFrame:
    """Return a per-atom table for one frame with only coordinate columns (and any keep_cols).
    Useful to strip charges/velocities/etc. while keeping XYZ.
    """
    ft = _frame_table(xh, int(frame)).copy()
    # filter atoms (optional)
    if atoms is not None:
        if isinstance(atoms, slice):
            idx = list(range(*atoms.indices(len(ft))))
        else:
            idx = [int(a) for a in atoms if 0 <= int(a) < len(ft)]
        ft = ft.iloc[idx].reset_index(drop=True)
    elif atom_types:
        tset = {str(t) for t in atom_types}
        ft = ft[ft["atom_type"].astype(str).isin(tset)].reset_index(drop=True)

    keep = [c for c in keep_cols if c in ft.columns]
    return ft[keep].copy()
...
def mean_squared_displacement(
    ...
    *,
    ...
    """Naive MSD (no PBC unwrapping). Computes <|r(t) - r(0)|^2> across selected atoms.
    Returns DataFrame: frame_index, iter, msd
    """
    dims = tuple(d for d in dims if d in ("x", "y", "z"))
    if not dims:
        raise ValueError("dims must include at least one of 'x','y','z'")
    ...
    fidx = list(sub_df.index)[::max(1, int(every))]
    if not fidx:
        return pd.DataFrame(columns=["frame_index", "iter", "msd"])
    ...
    if atoms is not None:
        if isinstance(atoms, slice):
            ...
            sel = [int(a) - 1 for a in atoms if 0 <= int(a) < coords0.shape[0]] #substract by 1 to convert 1-based to 0-based atom formatting
    elif atom_types:
        tset = {str(t) for t in atom_types}
    ...
    r0 = coords0[np.asarray(sel)[:, None], use_cols].astype(float)  # (n_sel, len(dims))

    rows: List[Dict[str, Any]] = []
    for i in fidx:
        fr = xh.frame(i)
        coords = fr["coords"][np.asarray(sel)[:, None], use_cols].astype(float)
        dr = coords - r0
        msd = float(np.mean(np.sum(dr * dr, axis=1))) if len(sel) else np.nan
        rows.append({
            "frame_index": int(i),
            "iter": int(fr.get("iter", i)),
            "msd": msd,
