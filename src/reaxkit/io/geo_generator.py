"""High-level utilities for generating and manipulating atomic structures.

This module provides a unified interface for creating, converting, and modifying
structures used in ReaxFF simulations. It includes:

  • Conversion of XYZ files to GEO/XTLGRF format (xtob), including sorting,
    CRYSTX handling, and full HETATM formatting.
  • Structure I/O wrappers built on ASE for reading/writing CIF, POSCAR,
    VASP, XYZ, and other formats.
  • Construction of surface slabs from bulk crystals using Miller indices.
  • Supercell generation through repetition or full 3×3 transformation
    matrices.
  • Hexagonal → orthorhombic cell orthogonalization via a standard a–b,
    a+b transformation.
  • A full Python reimplementation of the Fortran place2 algorithm for
    packing multiple randomly oriented molecular copies into a simulation box
    (with periodic boundary conditions and minimum-distance constraints).

These tools form the structure-generation layer of reaxkit and are intended to
support preprocessing, model construction, and geometry preparation for
ReaxFF simulations.
...
def xtob(
    ...
    """
    Convert an XYZ file to GEO (.geo / XTLGRF) format.

    Parameters
    ----------
    xyz_file : str or Path
        Input .xyz file path.
    geo_file : str or Path, optional
        Output .geo file path. Default is "geo".
    box_lengths : iterable of float, optional
        Periodic box dimensions (a, b, c) in Angstrom. REQUIRED for ReaxFF.
    box_angles : iterable of float, optional
        Periodic box angles (alpha, beta, gamma) in degrees.
    sort_by : {"x", "y", "z", "atom_type"} or None, optional
        If given, sort atoms by this column before writing.
    ascending : bool, default True
        Sort direction.
...
def read_structure(
    ...
    """
    Read a structure using ASE.

    Parameters
    ----------
    path : str or Path
        Input structure file. Can be CIF, POSCAR, VASP, etc.
    format : str, optional
        This is based on ASE format support as explained here:
        https://ase-lib.org/ase/io/io.html#ase.io.read
        ASE format string (e.g., "cif", "vasp"). If None, ASE guesses from file.
    index : int or str, default 0
        Which image to read if file contains multiple structures.
...
def build_surface(
    ...
    """
    Build a surface slab from a bulk structure.
    ...
    bulk : ase.Atoms
        Bulk structure.
    miller : (h, k, l)
        Miller indices of the surface, e.g. (0, 0, 1) or (1, 0, 0).
    layers : int
        Number of atomic layers in the slab.
    vacuum : float, default 15.0
        Vacuum thickness added along the surface normal (Å).
    center : bool, default True
        If True, center the slab along the surface-normal axis.
...
def make_supercell(
    ...
    """
    Create a supercell / transformed structure.

    Parameters
    ----------
    atoms : ase.Atoms
        Input structure (bulk or slab).
    transform : 3x3 integer matrix or (nx, ny, nz) tuple
        - If a tuple (nx, ny, nz), use repetition along a1, a2, a3.
        - If a 3×3 matrix (P), use ASE make_supercell(atoms, P).
...
def orthogonalize_hexagonal_cell(atoms: Atoms) -> Atoms:
    ...
    ----------
    atoms : ase.Atoms
        Structure with hexagonal cell (e.g., AlN, GaN, many 2D materials).

    Returns
    -------
    ase.Atoms
        New Atoms object with orthorhombic cell.
    """
    import numpy as np
...
def place2(
    ...
    *,
    ...
    automatically using the base structure only if provided.

    Parameters
    ----------
    insert_molecule : str or Path
        Structure to duplicate randomly (the "template molecule").
    base_structure : str or Path or None, optional
        Initial structure onto which molecules are inserted.
        If None, placement starts from an empty box.
    n_copies : int
        Number of copies of the insert_molecule to place.
    box_length_x, box_length_y, box_length_z : float
        Box lengths (Å).
    alpha, beta, gamma : float
        Unit cell angles (degrees).
    min_interatomic_distance : float
        Minimum allowed distance between any inserted atom and the current system.
    base_structure_placement_mode : {"as-is", "center", "origin"}, default "as-is"
        How to reposition base_structure before placement.
    max_placement_attempts_per_copy : int
        Maximum attempts for placing each molecule.
