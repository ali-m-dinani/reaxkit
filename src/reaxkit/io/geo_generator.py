"""XYZ → GEO converter (xtob) for ReaxFF/AMS geometry files.

This module reads a .xyz file and writes a .geo (XTLGRF) file.

XYZ format (assumed):

    117
    slab some optional text that we ignore
    Al       3.23458000       5.01663000      56.49674000
    ...

- Line 1: number of atoms (N)
- Line 2: descriptor line; ONLY the first token is used as descriptor, rest ignored
- Lines 3..: atom symbol, x, y, z, [optional extra columns that are ignored]

GEO / XTLGRF output format (simplified):

    XTLGRF 200
    DESCRP  Al2N2_w_001_water
    REMARK .bgf-file generated by xtob-python
    REMARK Structure sorted by x-coordinate (ascending)
...
Notes
-----
- We only use: atom index, atom type, x, y, z, repeated atom type, and dummy zeros.
- Extra fields in XYZ are ignored.
- Sorting is optional and reassigns atom indices after sorting.
"""
