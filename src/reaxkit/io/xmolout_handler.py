...
class XmoloutHandler(FileHandler):
    """Lightweight parser for ReaxFF **xmolout**.

    Outputs
    -------
    - ``dataframe()`` → per-frame summary with columns:
      ``num_of_atoms, iter, E_pot, a, b, c, alpha, beta, gamma`` (deduped by iter).
    - ``metadata()`` → dict with ``simulation_name, n_atoms, n_frames, has_time``.
    - ``frame(i)`` → dict with:
         ``index, iter, coords`` (``(n_atoms,3)`` float array), ``atom_types`` (list[str]).
      Any extra atom columns in the file are preserved per-frame in the internal tables.

    Notes
    -----
    Atom lines usually provide ``atom_type x y z``. If more columns exist, pass their
    names via ``extra_atom_cols=[...]`` to ``__init__``; otherwise they are auto‑named
    ``unknown_1, unknown_2, ...`` for that frame.
    """
