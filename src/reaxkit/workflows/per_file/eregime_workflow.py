...
`eregime.in` files, which define iteration-dependent electric-field schedules
in ReaxFF simulations.

It supports:
- Extracting electric-field components (e.g. E1, E2) versus iteration, with
  optional plotting and CSV export.
- Selecting subsets of data by row ranges for focused analysis.
...
from reaxkit.analysis.timeseries import EregimeSeriesRequest, EregimeSeriesTask
from reaxkit.cli.path import resolve_output_path
from reaxkit.core.alias import normalize_choice
from reaxkit.core.frame_utils import parse_frames, select_frames
from reaxkit.domain.data_models import EregimeData
...
def _eregime_get_task(args: argparse.Namespace) -> int:
    """Load one eregime column versus iteration and optionally export or plot it."""
    ...
    df = EregimeSeriesTask().run(
        data,
        EregimeSeriesRequest(field=args.column),
    ).table
    ...
    x_label = "iter"
    y_label = "value"
    canonical_label = normalize_choice(args.column)
    unit = unit_for(canonical_label) or ""
    ...
    if args.save:
        ...
        single_plot(
            ...
            title=title,
            xlabel=x_label,
            ylabel=f"{canonical_label} ({unit})" if unit else canonical_label,
            save=out,
        )
    ...
    if not (args.export or args.save or args.plot):
        print("No action chosen. Add one or more of: --plot, --save PATH, --export PATH")
        print(f"Available output columns: {', '.join(df.columns)}")

    return 0


def _task_gen_sin(args: argparse.Namespace) -> int:
    """Run the sinusoidal eregime generator task."""
    write_eregime_sinusoidal(
        args.output,
...
def _task_gen_pulse(args: argparse.Namespace) -> int:
    """Run the smooth-pulse eregime generator task."""
    write_eregime_smooth_pulse(
        args.output,
...
def _safe_build_func(expr: str) -> Callable[[float], float]:
    """Build func(t) from a simple expression like '0.003*sin(2*pi*t/100)'."""
    import math
    import numpy as np
...
def _task_gen_func(args: argparse.Namespace) -> int:
    """Run the function-based eregime generator task."""
...
def register_tasks(subparsers: argparse._SubParsersAction) -> None:
    """Register eregime workflow tasks."""
    p_get = subparsers.add_parser(
        "get",
        help="Get a column (like E1) vs iter and optionally plot, save, or export it.\n",
        description=(
            "Examples:\n"
            "  reaxkit eregime get --column E1 --export eregime_E1_vs_iter.csv "
            "--save eregime_E1_vs_iter.png\n"
        ),
    ...
    p_sin.add_argument("--max-magnitude", type=float, required=True, help="Peak amplitude (V/A).")
    ...
    p_sin.add_argument("--dc-offset", type=float, default=0.0, help="DC offset (V/A).")
    ...
    p_pulse.add_argument("--amplitude", type=float, required=True, help="Peak amplitude (V/A).")
    ...
    p_pulse.add_argument("--baseline", type=float, default=0.0, help="Baseline (V/A).")
