"""xmolout workflow for trajectory extraction, box dimensions, and trimming."""

from __future__ import annotations

import argparse
from typing import Iterable, Optional

import numpy as np
import pandas as pd

from reaxkit.analysis.timeseries.timeseries import (
    CellDimensionsRequest,
    CellDimensionsTask,
    TrajectoryCoordinateSeriesRequest,
    TrajectoryCoordinateSeriesTask,
)
from reaxkit.cli.path import resolve_output_path
from reaxkit.core.frame_utils import _select_frames, parse_atoms, parse_frames as _parse_frames
from reaxkit.engine.reaxff.adapter import _trajectory_from_xmolout_handler
from reaxkit.engine.reaxff.generators.xmolout_generator import write_xmolout_from_handler
from reaxkit.engine.reaxff.io.xmolout_handler import XmoloutHandler
from reaxkit.presentation.convert import convert_xaxis
from reaxkit.presentation.plot import multi_subplots, single_plot
from reaxkit.presentation.units import unit_for


def _parse_types(value: Optional[str]) -> set[str] | None:
    if value is None or str(value).strip() == "":
        return None
    parts = [part for chunk in str(value).split(",") for part in chunk.split()]
    return set(parts)


def _frames_from_args(xh: XmoloutHandler, args: argparse.Namespace) -> Iterable[int]:
    if getattr(args, "frames", None):
        selection = _parse_frames(args.frames)
        if selection is None:
            try:
                return range(xh.n_frames())
            except Exception:
                return range(len(xh.dataframe()))
        if isinstance(selection, slice):
            try:
                n_frames = xh.n_frames()
            except Exception:
                n_frames = len(xh.dataframe())
            start, stop, step = selection.indices(n_frames)
            return range(start, stop, step if step != 0 else 1)
        return [int(i) for i in selection]
    return _select_frames(xh, getattr(args, "start", None), getattr(args, "stop", None), getattr(args, "every", 1))


def _trajget_task(args: argparse.Namespace) -> int:
    xh = XmoloutHandler(args.file)
    frames = _frames_from_args(xh, args)

    atoms = parse_atoms(args.atoms)
    if atoms is not None:
        atoms = [atom - 1 for atom in atoms]

    types = _parse_types(args.atom_types)
    dims = args.dims or ["x", "y", "z"]
    dims = [dim for dim in dims if dim in ("x", "y", "z")]
    if not dims:
        raise ValueError("At least one of --dims x y z must be provided.")

    traj = _trajectory_from_xmolout_handler(xh)
    result = TrajectoryCoordinateSeriesTask().run(
        traj,
        TrajectoryCoordinateSeriesRequest(
            atom_ids=([atom + 1 for atom in atoms] if atoms is not None else None),
            atom_types=sorted(types) if types else None,
            dims=dims,
            format=args.format,
            frames=list(frames) if frames is not None else None,
        ),
    )
    df = result.table if result.table is not None else pd.DataFrame()

    if df.empty:
        print("No trajectory rows selected (check --atoms / --atom-types / frame range).")
        return 1

    if "frame_index" not in df.columns:
        raise KeyError("Expected 'frame_index' column in trajectory data.")

    xvals, xlabel = convert_xaxis(df["frame_index"].to_numpy(), args.xaxis)
    df = df.copy()
    df.insert(0, "xaxis", xvals)

    workflow_name = args.kind
    if args.export:
        out = resolve_output_path(args.export, workflow_name)
        df.to_csv(out, index=False)
        print(f"[Done] Exported trajectories to {out}")

    if args.save or args.plot:
        if args.format == "long" and len(dims) == 1:
            dim = dims[0]
            dff = df
            if "atom_id" in df.columns:
                atom_ids = df["atom_id"].unique().tolist()
                if len(atom_ids) > 1:
                    print(f"Multiple atoms detected: {atom_ids}. Plotting only atom_id={atom_ids[0]}.")
                atom_id = atom_ids[0]
                dff = df[df["atom_id"] == atom_id]

            title = f"{dim} vs {xlabel}"
            if "atom_id" in locals():
                title += f" (atom={atom_id})"

            y_unit = unit_for(args.yaxis) or unit_for(dim)
            save_target = resolve_output_path(args.save, workflow_name) if args.save else None
            single_plot(
                dff["xaxis"].values,
                dff[dim].values,
                title=title,
                xlabel=xlabel,
                ylabel=f"{dim} ({y_unit})" if y_unit else dim,
                save=save_target if args.save else None,
            )
            if args.plot and args.save:
                single_plot(
                    dff["xaxis"].values,
                    dff[dim].values,
                    title=title,
                    xlabel=xlabel,
                    ylabel=f"{dim} ({y_unit})" if y_unit else dim,
                    save=None,
                )
        else:
            print("Plotting is only supported for long-format with a single dim (x|y|z).")

    if not args.export and not args.save and not args.plot:
        print(df.head(10).to_string(index=False))

    return 0


def _boxdims_task(args: argparse.Namespace) -> int:
    xh = XmoloutHandler(args.file)
    frames = list(_frames_from_args(xh, args)) if getattr(args, "frames", None) else None

    sim = _trajectory_from_xmolout_handler(xh).simulation
    result = CellDimensionsTask().run(
        sim,
        CellDimensionsRequest(
            fields=("a", "b", "c", "alpha", "beta", "gamma"),
            frames=frames,
        ),
    )
    if not result.series:
        print("No box-dimension data found for the requested frames.")
        return 1

    metadata = result.metadata or {}
    frame_index = np.asarray(metadata.get("frame_index", np.empty((0,), dtype=int)), dtype=int)
    iterations = np.asarray(metadata.get("iterations", np.empty((0,), dtype=int)), dtype=int)
    df = pd.DataFrame({"frame_index": frame_index, "iter": iterations})
    for series in result.series:
        df[series.label] = np.asarray(series.y, dtype=float)

    xvals, xlabel = convert_xaxis(df["iter"].to_numpy(), args.xaxis)
    df = df.copy()
    df.insert(0, "x", xvals)

    workflow_name = args.kind
    if args.export:
        out = resolve_output_path(args.export, workflow_name)
        df.to_csv(out, index=False)
        print(f"[Done] Exported box-dimension table to {out}")

    if args.plot or args.save:
        ycols = [col for col in ("a", "b", "c") if col in df.columns]
        if not ycols:
            print("No a/b/c columns found in box-dimension table; nothing to plot.")
        else:
            subplots_data = [
                [{"x": df["x"].to_numpy(), "y": df[col].to_numpy(), "label": col}]
                for col in ycols
            ]
            save_target = resolve_output_path(args.save, workflow_name) if args.save else None
            multi_subplots(
                subplots=subplots_data,
                title=f"Box dimensions vs {xlabel}",
                xlabel=xlabel,
                ylabel="Length (A)",
                legend=True,
                save=save_target if (args.save or args.plot) else None,
            )

    if not args.export and not args.save and not args.plot:
        print(df.head(10).to_string(index=False))

    return 0


def _trim_task(args: argparse.Namespace) -> int:
    xh = XmoloutHandler(args.file)
    out_path = args.output or "xmolout_trimmed"
    write_xmolout_from_handler(xh, out_path, include_extras=False)
    print(f"[Done] Wrote trimmed xmolout (type + x,y,z only) to {out_path}")
    return 0


def _add_common_xmolout_io_args(parser: argparse.ArgumentParser, *, include_plot: bool = False) -> None:
    parser.add_argument("--file", default="xmolout", help="Path to xmolout file.")
    if include_plot:
        parser.add_argument("--plot", action="store_true", help="Show plot interactively.")
    parser.add_argument("--save", default=None, help="Path to save plot image.")
    parser.add_argument("--export", default=None, help="Path to export CSV data.")


def register_tasks(subparsers: argparse._SubParsersAction) -> None:
    pt = subparsers.add_parser(
        "trajget",
        help="Get atom trajectories per frame (single or multiple atoms).",
        description=(
            "Get atom trajectories from xmolout.\n\n"
            "Examples:\n"
            "  reaxkit xmolout trajget --atoms 1 --dims z --xaxis time --save atom1_z.png --export atom1_z.csv\n"
            "  reaxkit xmolout trajget --atom-types Al --dims x y z --format wide --export Al_all_dims_traj.csv\n"
            "  reaxkit xmolout trajget --atom-types Al --frames 10:200:10 --dims z --export Al_z_dim_traj.csv\n"
        ),
        formatter_class=argparse.RawTextHelpFormatter,
    )
    _add_common_xmolout_io_args(pt, include_plot=True)
    pt.add_argument("--atoms", default=None, help="Comma/space separated 1-based atom indices, e.g. '1,5,12'.")
    pt.add_argument("--dims", nargs="+", default=None, choices=["x", "y", "z"], help="Coordinate dimensions to include (default: x y z).")
    pt.add_argument("--xaxis", default="frame", choices=["iter", "frame", "time"], help="Quantity on x-axis (default: frame).")
    pt.add_argument("--frames", default=None, help="Frame selector: 'start:stop[:step]' or 'i,j,k' (default: all).")
    pt.add_argument("--atom-types", "--types", dest="atom_types", default=None, help="Comma/space separated atom types, e.g. 'Al,N'.")
    pt.add_argument("--format", choices=["long", "wide"], default="long", help="Output table layout: long or wide (default: long).")
    pt.set_defaults(_run=_trajget_task)

    pb = subparsers.add_parser(
        "boxdims",
        help="Get box/cell dimensions per frame and optionally plot them.",
        description=(
            "Get box/cell dimensions from xmolout.\n\n"
            "Examples:\n"
            "  reaxkit xmolout boxdims --frames 0:500:5 --xaxis time --export box_dims.csv\n"
            "  reaxkit xmolout boxdims --frames 0:100:10 --xaxis iter --save box_dim_plots.png\n"
        ),
        formatter_class=argparse.RawTextHelpFormatter,
    )
    _add_common_xmolout_io_args(pb, include_plot=True)
    pb.add_argument("--frames", default=None, help="Frame selector: 'start:stop[:step]' or 'i,j,k'.")
    pb.add_argument("--xaxis", choices=["frame", "iter", "time"], default="frame", help="Quantity on x-axis (default: frame).")
    pb.set_defaults(_run=_boxdims_task)

    ptr = subparsers.add_parser(
        "trim",
        help="Write a trimmed copy of xmolout with only atom_type and x,y,z.",
        description=(
            "Trim xmolout to a lighter file with atom type and coordinates only.\n\n"
            "Example:\n"
            "  reaxkit xmolout trim --file xmolout --output xmolout_trimmed\n"
        ),
    )
    ptr.add_argument("--file", default="xmolout", help="Input xmolout file.")
    ptr.add_argument("--output", default="xmolout_trimmed", help="Output trimmed xmolout file.")
    ptr.set_defaults(_run=_trim_task)
