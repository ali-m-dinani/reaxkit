"""Sanity check for ActiveSiteStructuralTask via AnalysisExecutor."""
...
def _run_and_save() -> Path:
    ...
    assert tuple(result.tract_table.columns) == TRACT_STRUCTURAL_COLUMNS
    ...
    metadata_path = task_artifacts_dir / "active_site_structural_summary.txt"
    csv_path = task_artifacts_dir / "active_site_structural.csv"
    head_path = task_artifacts_dir / "active_site_structural_head.txt"
...
def test_active_site_structural_saves_artifacts() -> None:
    if not RUN_DIR.exists():
        pytest.skip(f"RUN_DIR does not exist: {RUN_DIR}")
    out_dir = _run_and_save()
    assert (out_dir / "active_site_structural_summary.txt").exists()
    assert (out_dir / "active_site_structural.csv").exists()
    assert (out_dir / "active_site_structural_head.txt").exists()
