"""Sanity check for BondEventsTask via AnalysisExecutor."""
...
from reaxkit.analysis.connectivity.connectivity import BondEventsRequest, BondEventsTask
...
def _run_and_save() -> Path:
    ...
    metadata_path = task_artifacts_dir / "bond_events_summary.txt"
    csv_path = task_artifacts_dir / "bond_events.csv"
    head_path = task_artifacts_dir / "bond_events_head.txt"
    ...
    metadata_path.write_text(
        "\n".join(
            [
                ...
                f"Request hysteresis: {result.request.hysteresis}",
                f"Request smooth: {result.request.smooth}",
...
def test_bond_events_saves_artifacts() -> None:
    if not RUN_DIR.exists():
        pytest.skip(f"RUN_DIR does not exist: {RUN_DIR}")
    out_dir = _run_and_save()
    assert (out_dir / "bond_events_summary.txt").exists()
    assert (out_dir / "bond_events.csv").exists()
    assert (out_dir / "bond_events_head.txt").exists()
