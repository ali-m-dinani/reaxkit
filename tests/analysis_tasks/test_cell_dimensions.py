"""Sanity check for CellDimensionsTask via AnalysisExecutor."""
...
from reaxkit.analysis.timeseries.timeseries import CellDimensionsRequest, CellDimensionsTask
...
def _run_and_save() -> Path:
    ...
    metadata_path = task_artifacts_dir / "cell_dimensions_summary.txt"
    csv_path = task_artifacts_dir / "cell_dimensions.csv"
    head_path = task_artifacts_dir / "cell_dimensions_head.txt"
    ...
    metadata_path.write_text(
        "\n".join(
            [
                ...
                f"Request fields: {list(result.request.fields)}",
...
def test_cell_dimensions_saves_artifacts() -> None:
    if not RUN_DIR.exists():
        pytest.skip(f"RUN_DIR does not exist: {RUN_DIR}")
    out_dir = _run_and_save()
    assert (out_dir / "cell_dimensions_summary.txt").exists()
    assert (out_dir / "cell_dimensions.csv").exists()
    assert (out_dir / "cell_dimensions_head.txt").exists()
