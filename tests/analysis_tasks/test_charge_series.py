"""Sanity check for ChargeSeriesTask via AnalysisExecutor."""
...
from reaxkit.analysis.timeseries.timeseries import ChargeSeriesRequest, ChargeSeriesTask
...
def _run_and_save() -> Path:
    ...
    assert {"frame_index", "iter", "atom_id", "charge"}.issubset(set(result.table.columns))

    metadata_path = task_artifacts_dir / "charge_series_summary.txt"
    csv_path = task_artifacts_dir / "charge_series.csv"
    head_path = task_artifacts_dir / "charge_series_head.txt"
    ...
    metadata_path.write_text(
        "\n".join(
            [
                ...
                f"Request atom_ids: {list(result.request.atom_ids)}",
...
def test_charge_series_saves_artifacts() -> None:
    if not RUN_DIR.exists():
        pytest.skip(f"RUN_DIR does not exist: {RUN_DIR}")
    out_dir = _run_and_save()
    assert (out_dir / "charge_series_summary.txt").exists()
    assert (out_dir / "charge_series.csv").exists()
    assert (out_dir / "charge_series_head.txt").exists()
