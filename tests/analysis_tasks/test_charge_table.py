"""Sanity check for ChargeTableTask via AnalysisExecutor."""
...
def _run_and_save() -> Path:
    ...
    assert {"frame_index", "iter", "atom_id", "atom_type", "charge"}.issubset(set(result.table.columns))
    ...
    metadata_path = task_artifacts_dir / "charge_table_summary.txt"
    csv_path = task_artifacts_dir / "charge_table.csv"
    head_path = task_artifacts_dir / "charge_table_head.txt"
    ...
    metadata_path.write_text(
        "\n".join(
            [
                ...
                f"Request atom_ids: {list(result.request.atom_ids) if result.request.atom_ids is not None else None}",
                f"Request atom_types: {list(result.request.atom_types) if result.request.atom_types is not None else None}",
...
def test_charge_table_saves_artifacts() -> None:
    if not RUN_DIR.exists():
        pytest.skip(f"RUN_DIR does not exist: {RUN_DIR}")
    out_dir = _run_and_save()
    assert (out_dir / "charge_table_summary.txt").exists()
    assert (out_dir / "charge_table.csv").exists()
    assert (out_dir / "charge_table_head.txt").exists()
