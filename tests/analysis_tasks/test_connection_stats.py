"""Sanity check for ConnectionStatsTask via AnalysisExecutor."""
...
from reaxkit.analysis.connectivity.connectivity import ConnectionStatsRequest, ConnectionStatsTask
...
def _run_and_save() -> Path:
    ...
    metadata_path = task_artifacts_dir / "connection_stats_summary.txt"
    csv_path = task_artifacts_dir / "connection_stats.csv"
    head_path = task_artifacts_dir / "connection_stats_head.txt"
    ...
    metadata_path.write_text(
        "\n".join(
            [
                ...
                f"Request how: {result.request.how}",
            ]
        )
        + "\n",
        encoding="utf-8",
...
def test_connection_stats_saves_artifacts() -> None:
    if not RUN_DIR.exists():
        pytest.skip(f"RUN_DIR does not exist: {RUN_DIR}")
    out_dir = _run_and_save()
    assert (out_dir / "connection_stats_summary.txt").exists()
    assert (out_dir / "connection_stats.csv").exists()
    assert (out_dir / "connection_stats_head.txt").exists()
