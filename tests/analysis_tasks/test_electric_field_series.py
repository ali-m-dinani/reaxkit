"""Sanity check for ElectricFieldSeriesTask via AnalysisExecutor."""
...
from reaxkit.analysis.timeseries.timeseries import ElectricFieldSeriesRequest, ElectricFieldSeriesTask
...
def _run_and_save() -> Path:
    ...
    try:
        ...
        if "expected data type ElectricFieldData, got NoneType" in str(exc):
            pytest.skip("ElectricFieldData is not available for this run_dir.")
        raise
    assert result.request == request
    assert {"frame_index", "iter", "component", "value"}.issubset(set(result.table.columns))

    metadata_path = task_artifacts_dir / "electric_field_series_summary.txt"
    csv_path = task_artifacts_dir / "electric_field_series.csv"
    head_path = task_artifacts_dir / "electric_field_series_head.txt"
    ...
    metadata_path.write_text(
        "\n".join(
            [
                ...
                f"Request components: {list(result.request.components)}",
                f"Request field_kind: {result.request.field_kind}",
...
def test_electric_field_series_saves_artifacts() -> None:
    if not RUN_DIR.exists():
        pytest.skip(f"RUN_DIR does not exist: {RUN_DIR}")
    out_dir = _run_and_save()
    assert (out_dir / "electric_field_series_summary.txt").exists()
    assert (out_dir / "electric_field_series.csv").exists()
    assert (out_dir / "electric_field_series_head.txt").exists()
