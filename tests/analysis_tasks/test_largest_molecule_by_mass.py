"""Sanity check for LargestMoleculeByMassTask via AnalysisExecutor."""
...
def _run_and_save() -> Path:
    ...
    assert {"frame_index", "iter", "molecular_formula", "freq", "molecular_mass"}.issubset(
        set(result.table.columns)
    )

    metadata_path = task_artifacts_dir / "largest_molecule_by_mass_summary.txt"
    csv_path = task_artifacts_dir / "largest_molecule_by_mass.csv"
    head_path = task_artifacts_dir / "largest_molecule_by_mass_head.txt"
...
def test_largest_molecule_by_mass_saves_artifacts() -> None:
    if not RUN_DIR.exists():
        pytest.skip(f"RUN_DIR does not exist: {RUN_DIR}")
    out_dir = _run_and_save()
    assert (out_dir / "largest_molecule_by_mass_summary.txt").exists()
    assert (out_dir / "largest_molecule_by_mass.csv").exists()
    assert (out_dir / "largest_molecule_by_mass_head.txt").exists()
