"""Sanity check for MolecularFrequencySeriesTask via AnalysisExecutor."""
...
from reaxkit.analysis.timeseries.timeseries import MolecularFrequencySeriesRequest, MolecularFrequencySeriesTask
...
def _run_and_save() -> Path:
    ...
    assert {"frame_index", "iter", "molecules", "freq", "molecular_mass"}.issubset(set(result.table.columns))

    metadata_path = task_artifacts_dir / "molecular_frequency_series_summary.txt"
    csv_path = task_artifacts_dir / "molecular_frequency_series.csv"
    head_path = task_artifacts_dir / "molecular_frequency_series_head.txt"
    ...
    metadata_path.write_text(
        "\n".join(
            [
                ...
                f"Request molecules: {list(result.request.molecules)}",
...
def test_molecular_frequency_series_saves_artifacts() -> None:
    if not RUN_DIR.exists():
        pytest.skip(f"RUN_DIR does not exist: {RUN_DIR}")
    out_dir = _run_and_save()
    assert (out_dir / "molecular_frequency_series_summary.txt").exists()
    assert (out_dir / "molecular_frequency_series.csv").exists()
    assert (out_dir / "molecular_frequency_series_head.txt").exists()
