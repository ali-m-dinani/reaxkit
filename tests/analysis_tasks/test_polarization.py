"""Sanity check for PolarizationTask via AnalysisExecutor."""
...
from reaxkit.analysis.electrostatics.electrostatics import PolarizationRequest, PolarizationTask
...
def _run_and_save() -> Path:
    ...
    metadata_path = task_artifacts_dir / "polarization_summary.txt"
    csv_path = task_artifacts_dir / "polarization.csv"
    head_path = task_artifacts_dir / "polarization_head.txt"
    ...
    metadata_path.write_text(
        "\n".join(
            [
                ...
                f"Request volume_method: {result.request.volume_method}",
...
def test_polarization_saves_artifacts() -> None:
    if not RUN_DIR.exists():
        pytest.skip(f"RUN_DIR does not exist: {RUN_DIR}")
    out_dir = _run_and_save()
    assert (out_dir / "polarization_summary.txt").exists()
    assert (out_dir / "polarization.csv").exists()
    assert (out_dir / "polarization_head.txt").exists()
