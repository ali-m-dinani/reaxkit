"""Sanity check for StructureSummaryTask via AnalysisExecutor."""
...
def _run_and_save() -> Path:
    ...
    try:
        ...
        msg = str(exc)
        if (
            "expected data type GeometrySummaryData, got NoneType" in msg
            or "Failed to load required data 'GeometrySummaryData'" in msg
        ):
            pytest.skip("GeometrySummaryData is not available for this run_dir.")
        raise

    assert result.request == request
    assert {"identifier", "Emin", "iteration", "enthalpy", "volume", "density"}.issubset(set(result.table.columns))

    metadata_path = task_artifacts_dir / "structure_summary_summary.txt"
    csv_path = task_artifacts_dir / "structure_summary.csv"
    head_path = task_artifacts_dir / "structure_summary_head.txt"
...
def test_structure_summary_saves_artifacts() -> None:
    if not RUN_DIR.exists():
        pytest.skip(f"RUN_DIR does not exist: {RUN_DIR}")
    out_dir = _run_and_save()
    assert (out_dir / "structure_summary_summary.txt").exists()
    assert (out_dir / "structure_summary.csv").exists()
    assert (out_dir / "structure_summary_head.txt").exists()
