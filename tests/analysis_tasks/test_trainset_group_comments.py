"""Sanity check for TrainsetGroupCommentsTask via AnalysisExecutor."""
...
def _run_and_save() -> Path:
    ...
    assert {"section", "group_comment", "line_number", "count"}.issubset(set(result.table.columns))
    line_no = result.table["line_number"].dropna().to_numpy()
    if len(line_no) > 1:
        assert (line_no[1:] >= line_no[:-1]).all()

    metadata_path = task_artifacts_dir / "trainset_group_comments_summary.txt"
    csv_path = task_artifacts_dir / "trainset_group_comments.csv"
    head_path = task_artifacts_dir / "trainset_group_comments_head.txt"
...
def test_trainset_group_comments_saves_artifacts() -> None:
    if not RUN_DIR.exists():
        pytest.skip(f"RUN_DIR does not exist: {RUN_DIR}")
    out_dir = _run_and_save()
    assert (out_dir / "trainset_group_comments_summary.txt").exists()
    assert (out_dir / "trainset_group_comments.csv").exists()
    assert (out_dir / "trainset_group_comments_head.txt").exists()
