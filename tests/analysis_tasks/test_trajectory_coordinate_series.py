"""Sanity check for TrajectoryCoordinateSeriesTask via AnalysisExecutor."""
...
def _run_and_save() -> Path:
    ...
    metadata_path = task_artifacts_dir / "trajectory_coordinate_series_summary.txt"
    csv_path = task_artifacts_dir / "trajectory_coordinate_series.csv"
    head_path = task_artifacts_dir / "trajectory_coordinate_series_head.txt"
    ...
    metadata_path.write_text(
        "\n".join(
            [
                ...
                f"Request dims: {list(result.request.dims)}",
...
def test_trajectory_coordinate_series_saves_artifacts() -> None:
    if not RUN_DIR.exists():
        pytest.skip(f"RUN_DIR does not exist: {RUN_DIR}")
    out_dir = _run_and_save()
    assert (out_dir / "trajectory_coordinate_series_summary.txt").exists()
    assert (out_dir / "trajectory_coordinate_series.csv").exists()
    assert (out_dir / "trajectory_coordinate_series_head.txt").exists()
