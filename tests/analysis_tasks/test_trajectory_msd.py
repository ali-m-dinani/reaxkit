"""Sanity check for MSDTask via AnalysisExecutor."""
...
def _run_and_save() -> Path:
    ...
    metadata_path = task_artifacts_dir / "trajectory_msd_summary.txt"
    csv_path = task_artifacts_dir / "trajectory_msd.csv"
    head_path = task_artifacts_dir / "trajectory_msd_head.txt"
...
def test_trajectory_msd_saves_artifacts() -> None:
    if not RUN_DIR.exists():
        pytest.skip(f"RUN_DIR does not exist: {RUN_DIR}")
    out_dir = _run_and_save()
    assert (out_dir / "trajectory_msd_summary.txt").exists()
    assert (out_dir / "trajectory_msd.csv").exists()
    assert (out_dir / "trajectory_msd_head.txt").exists()
