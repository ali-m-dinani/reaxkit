"""Sanity check for RDFPropertyTask via AnalysisExecutor."""
...
from reaxkit.analysis.trajectory.rdf import RDFPropertyRequest, RDFPropertyTask
...
def _run_and_save() -> Path:
    ...
    assert {"frame_index", "iter"}.issubset(set(result.table.columns))
    expected_property_cols = {
        "first_peak": {"r_first_peak", "g_first_peak"},
        "dominant_peak": {"r_peak", "g_peak"},
        "area": {"area"},
        "excess_area": {"excess_area"},
    }
    assert expected_property_cols[request.property].issubset(set(result.table.columns))

    metadata_path = task_artifacts_dir / "trajectory_rdf_property_summary.txt"
    csv_path = task_artifacts_dir / "trajectory_rdf_property.csv"
    head_path = task_artifacts_dir / "trajectory_rdf_property_head.txt"
    ...
    metadata_path.write_text(
        "\n".join(
            [
                ...
                f"Request property: {result.request.property}",
...
def test_trajectory_rdf_property_saves_artifacts() -> None:
    ...
    assert (out_dir / "trajectory_rdf_property_summary.txt").exists()
    assert (out_dir / "trajectory_rdf_property.csv").exists()
    assert (out_dir / "trajectory_rdf_property_head.txt").exists()
